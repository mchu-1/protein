...
def _add_local_modules(image: modal.Image) -> modal.Image:
    ...
    return (
        image.add_local_file("common.py", remote_path="/root/common.py")
...
class TargetProtein(BaseModel):
    ...
    pdb_path: Optional[str] = Field(
        default=None, description="Path to downloaded PDB file"
    )
    chain_id: Optional[str] = Field(
        default=None, description="Chain ID mapped from entity"
    )
...
class RFDiffusionConfig(BaseModel):
    ...
    num_diffusion_steps: int = Field(
        default=50, description="Number of diffusion steps"
    )
...
class ESMFoldConfig(BaseModel):
    ...
    max_rmsd: float = Field(
        default=2.5, description="Max RMSD to RFDiffusion backbone (consistency)"
    )
    save_predictions: bool = Field(
        default=False, description="Save ESMFold PDB predictions"
    )
...
class Boltz2Config(BaseModel):
    ...
    max_pae_interaction: float = Field(
        default=1.5, description="Max PAE at hotspots (Anchor Lock)"
    )
    min_ptm_binder: float = Field(
        default=0.80, description="Min pTM for binder-only (Fold Quality)"
    )
    max_rmsd: float = Field(
        default=2.5, description="Max RMSD vs RFDiffusion backbone (Self-Consistency)"
    )
...
class FoldSeekConfig(BaseModel):
    ...
    database: str = Field(
        default="pdb100", description="Database to search (pdb100, afdb50, etc.)"
    )
    ...
    cache_results: bool = Field(
        default=True,
        description="Cache decoy results by target (saves repeated FoldSeek calls)",
    )
...
class Chai1Config(BaseModel):
    ...
    min_chain_pair_iptm: float = Field(
        default=0.5, description="chain_pair_iptm threshold for cross-reactivity"
    )
    num_recycles: int = Field(default=3, description="Number of recycling iterations")
    tiered_checking: bool = Field(
        default=True,
        description="Use tiered decoy checking (Tier 1/2/3) for cost savings",
    )
    tier1_min_tm: float = Field(
        default=0.7, description="Min TM-score for Tier 1 (highest risk)"
    )
...
class ClusterConfig(BaseModel):
    ...
    tm_threshold: float = Field(
        default=0.7, description="TM-score threshold for clustering"
    )
    select_best: bool = Field(
        default=True, description="Select best representative per cluster"
    )
...
class NoveltyConfig(BaseModel):
    ...
    max_evalue: float = Field(
        default=1e-6, description="Max E-value to consider a hit (lower = stricter)"
    )
    enabled: bool = Field(default=True, description="Enable novelty filtering")
    auto_download: bool = Field(
        default=True, description="Auto-download UniRef50 database if not present"
    )
...
class ScoringWeights(BaseModel):
    ...
    alpha: float = Field(
        default=1.0, description="Weight for interface pLDDT (specificity)"
    )
    beta: float = Field(
        default=0.5, description="Weight for max decoy affinity (selectivity penalty)"
    )
...
class BackboneFilterConfig(BaseModel):
    ...
    min_score: float = Field(
        default=0.4, description="Min RFDiffusion confidence score"
    )
    max_keep: Optional[int] = Field(
        default=None, description="Max backbones to keep (None = no limit)"
    )
...
class AdaptiveGenerationConfig(BaseModel):
    ...
    min_validated_candidates: int = Field(
        default=3, description="Stop when this many pass Boltz-2"
    )
    batch_size: int = Field(
        default=4, description="Backbones to generate per micro-batch (GPU-efficient)"
    )
...
class SolubilityFilterConfig(BaseModel):
    ...
    min_abs_charge_ph7: float = Field(
        default=3.0,
        description="Minimum |net charge| at pH 7 (repulsion to prevent clumping)",
    )
    forbidden_pi_min: float = Field(
        default=6.0,
        description="Lower bound of forbidden pI zone (dead zone near pH 7.4)",
    )
    forbidden_pi_max: float = Field(
        default=8.0,
        description="Upper bound of forbidden pI zone (dead zone near pH 7.4)",
    )
...
class StructuralMemoizationConfig(BaseModel):
    ...
    similarity_threshold: float = Field(
        default=0.9, description="3Di similarity threshold to consider as 'twin'"
    )
    cache_ttl_hours: int = Field(
        default=168, description="TTL for structural cache (default: 7 days)"
    )
...
class BeamPruningConfig(BaseModel):
    ...
    max_tree_nodes: int = Field(
        default=500, description="Maximum total nodes in state tree"
    )
    sequences_per_backbone: int = Field(
        default=5, description="Max sequences to spawn per backbone"
    )
    predictions_per_sequence: int = Field(
        default=1, description="Max predictions per sequence (usually 1)"
    )
    prune_by_score: bool = Field(
        default=True, description="Prune lowest-scoring siblings when limit hit"
    )
...
class StickinessFilterConfig(BaseModel):
    ...
    max_sap_score: float = Field(
        default=0.50,
        description="Maximum SAP score. Relaxed to 0.50 to allow CDR-like loops.",
    )
    hydrophobic_residues: str = Field(
        default="AVILMFYW", description="Residues considered hydrophobic"
    )
...
class HardwareConfig(BaseModel):
    ...
    boltz2_gpu: str = Field(
        default="A100", description="GPU for Boltz-2 (A100=40GB, A100-80GB=80GB)"
    )
    chai1_gpu: str = Field(
        default="A100", description="GPU for Chai-1 (A100=40GB, A100-80GB=80GB)"
    )
...
class StageLimits(BaseModel):
    ...
    timeout_seconds: Optional[int] = Field(
        default=None, description="Max seconds for this stage (None = use default)"
    )
...
class PipelineConfig(BaseModel):
    ...
    mode: GenerationMode = Field(
        default=GenerationMode.BIND, description="Semantic generation mode"
    )
    ...
    esmfold: ESMFoldConfig = Field(
        default_factory=ESMFoldConfig,
        description="ESMFold orthogonal validation (gatekeeper)",
    )
    ...
    cluster: ClusterConfig = Field(
        default_factory=ClusterConfig, description="TM-score clustering for diversity"
    )
    novelty: NoveltyConfig = Field(
        default_factory=NoveltyConfig, description="Novelty check vs UniRef50"
    )
    scoring: ScoringWeights = Field(default_factory=ScoringWeights)

    # Hardware configuration
    hardware: HardwareConfig = Field(
        default_factory=HardwareConfig, description="GPU hardware selection"
    )

    # Cost optimization configs
    backbone_filter: BackboneFilterConfig = Field(
        default_factory=BackboneFilterConfig,
        description="Backbone quality pre-screening",
    )
    adaptive: AdaptiveGenerationConfig = Field(
        default_factory=AdaptiveGenerationConfig,
        description="Adaptive generation with early termination",
    )

    # New optimizations based on state tree
    solubility_filter: SolubilityFilterConfig = Field(
        default_factory=SolubilityFilterConfig,
        description="Lookahead solubility filtering",
    )
    structural_memoization: StructuralMemoizationConfig = Field(
        default_factory=StructuralMemoizationConfig,
        description="3Di structural hashing for deduplication",
    )
    beam_pruning: BeamPruningConfig = Field(
        default_factory=BeamPruningConfig,
        description="Greedy beam pruning to limit tree size",
    )
    stickiness_filter: StickinessFilterConfig = Field(
        default_factory=StickinessFilterConfig, description="SAP stickiness check"
    )

    # Stage limits (timeouts per stage)
    limits: PipelineLimits = Field(
        default_factory=PipelineLimits, description="Per-stage timeout limits"
    )
...
class BackboneDesign(BaseModel):
    ...
    binder_chain_id: str = Field(
        default="B", description="Chain ID of the designed binder"
    )
    rfdiffusion_score: Optional[float] = Field(
        None, description="RFDiffusion confidence"
    )
...
class SequenceDesign(BaseModel):
    ...
    backbone_chain_id: str = Field(
        ..., description="Chain ID of the binder in the backbone PDB (e.g., 'A')"
    )
    binder_chain_id: str = Field(
        default="B",
        description="Chain ID of the designed binder (deprecated, use backbone_chain_id)",
    )
    recovery: Optional[float] = Field(
        None, description="Sequence recovery if applicable"
    )
    backbone_pdb: Optional[str] = Field(
        None, description="Path to RFDiffusion backbone PDB (for RMSD)"
    )
...
class StructurePrediction(BaseModel):
    ...
    pae_interaction: Optional[float] = Field(
        None, description="Min PAE at hotspot residues (Anchor Lock)"
    )
    ptm_binder: Optional[float] = Field(
        None, description="pTM for binder-only (Fold Quality)"
    )
    rmsd_to_design: Optional[float] = Field(
        None, description="RMSD vs RFDiffusion backbone (Self-Consistency)"
    )
    binder_chain_id: str = Field(
        default="B", description="Chain ID of the designed binder"
    )
...
class CrossReactivityResult(BaseModel):
    ...
    predicted_affinity: float = Field(
        ..., description="Predicted binding affinity (lower = tighter)"
    )
    ...
    iptm: Optional[float] = Field(None, description="ipTM score")
    chain_pair_iptm: Optional[float] = Field(
        None, description="Chain-pair ipTM (off-target threshold: >0.5)"
    )
...
class BinderCandidate(BaseModel):
    ...
    backbone_design: BackboneDesign = Field(
        ..., description="Backbone generation result"
    )
    sequence_design: SequenceDesign = Field(..., description="Sequence design result")
    structure_prediction: StructurePrediction = Field(
        ..., description="Structure validation result"
    )
    ...
    specificity_score: float = Field(
        ..., description="Target binding score (higher = better, Boltz-2)"
    )
    chai1_specificity_score: Optional[float] = Field(
        None, description="Target binding score (higher = better, Chai-1)"
    )
    selectivity_score: float = Field(
        ..., description="Off-target avoidance (higher = better)"
    )
    ...
    def compute_final_score(self, alpha: float = 1.0, beta: float = 0.5) -> float:
        ...
        if (
            self.chai1_specificity_score is not None
            and self.chai1_specificity_score > 0
        ):
...
class PipelineResult(BaseModel):
    ...
    candidates: list[BinderCandidate] = Field(
        ..., description="All validated candidates"
    )
    top_candidates: list[BinderCandidate] = Field(
        ..., description="Top-ranked candidates"
    )
...
def get_entity_chain_mapping(pdb_id: str) -> dict[int, list[str]]:
    ...
    try:
            ...
            for entity_id in polymer_entity_ids:
                ...
                try:
                    with urllib.request.urlopen(
                        ...
                        entity_container = entity_data.get(
        ...
        raise ValueError(
            f"Could not fetch entity mapping for PDB {pdb_id}: HTTP {e.code}"
        )
    except Exception as e:
    ...
    try:
        ...
        with urllib.request.urlopen(url, timeout=10) as response:
            ...
            info = {
                ...
                "description": data.get("rcsb_polymer_entity", {}).get(
                    "pdbx_description", ""
                ),
                "uniprot_ids": data.get(
                    "rcsb_polymer_entity_container_identifiers", {}
                ).get("uniprot_ids", []),
            }
            return info
        ...
        raise ValueError(
            f"Could not fetch entity {entity_id} info for PDB {pdb_id}: {e}"
        )
