...
def filter_sequences_by_solubility(
    ...
    if n_filtered > 0:
        print(
            f"Solubility filter: {n_total} → {n_passed} sequences ({n_filtered} filtered)"
        )
        for r in results:
...
def filter_redundant_backbones(
    ...
    for backbone in backbones:
        ...
        for existing_id, existing_fp in all_fps.items():
            ...
            if similarity >= config.similarity_threshold:
                ...
                skipped_count += 1
                print(
                    f"  ✗ {backbone.design_id}: structural twin of {existing_id} (sim={similarity:.2f})"
                )
                break
    ...
    if skipped_count > 0:
        print(
            f"Structural memoization: {len(backbones)} → {len(unique_backbones)} backbones ({skipped_count} twins skipped)"
        )
...
class BeamPruner:
    ...
    def prune_sequences(
        ...
        print(
            f"  Beam pruning ({backbone_id}): {len(sequences)} → {max_seqs} sequences"
        )
...
def filter_sequences_by_stickiness(
    ...
    if n_filtered > 0:
        print(
            f"Stickiness filter: {n_total} → {n_passed} sequences ({n_filtered} filtered)"
        )
        for r in results:
            if not r.passes_filter:
                print(f"  ✗ {r.sequence_id}: {r.rejection_reason}")
