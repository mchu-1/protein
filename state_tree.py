...
class PipelineStateTree:
    ...
    STAGE_RESOURCES = {
        ...
        StageType.STRUCTURAL_MEMOIZATION: {
            "gpu": None,
            "cpu_cores": 1,
            "memory_gib": 2,
        },
    ...
    def set_status(
        self, node_id: str, status: NodeStatus, error: Optional[str] = None
    ) -> None:
    ...
    def set_ceilings(
        self, node_id: str, ceiling_cost_usd: float, ceiling_timing_sec: float
    ) -> None:
    ...
    def get_batch_efficiency(self) -> dict[str, Any]:
        ...
        return {
            ...
            "cold_start_amortization_pct": (cold_starts_avoided / total_items * 100)
            if total_items > 0
            else 0,
        }
    ...
    def get_live_optimization_stats(self) -> dict[str, Any]:
        ...
        optimization_savings_pct = (
            (saved_cost / (actual_cost + saved_cost) * 100)
            if (actual_cost + saved_cost) > 0
            else 0
        )
        ...
        under_ceiling_pct = (
            ((ceiling_cost - actual_cost) / ceiling_cost * 100)
            if ceiling_cost > 0
            else 0
        )
        ...
        return {
            ...
            "optimizations": {
                ...
                "total_optimized": solubility_filtered
                + stickiness_filtered
                + beam_pruned
                + cache_hits
                + structural_twins,
    ...
    def print_live_stats(self) -> None:
        ...
        nodes = stats["nodes"]
        print(
            f"Nodes: {nodes['completed']}/{nodes['total']} complete, {nodes['running']} running, {nodes['pending']} pending"
        )
        print(
            f"  Filtered: {nodes['filtered']} | Skipped: {nodes['skipped']} | Failed: {nodes['failed']}"
        )
        ...
        print(
            f"  Saved by optimizations: ${costs['saved_usd']:.3f} ({costs['optimization_savings_pct']:.0f}%)"
        )
        ...
        if batch.get("total_batches", 0) > 0:
            print("\nBatching:")
            print(
                f"  Batches: {batch['total_batches']} (avg size: {batch['avg_batch_size']:.1f})"
            )
    ...
    def get_children(self, node_id: str) -> list[NodeData]:
        ...
        if node_id not in self.graph:
            return []
        return [
            self.graph.nodes[child]["data"] for child in self.graph.successors(node_id)
        ]
    ...
    def summary(self) -> str:
        ...
        if ceiling_timing > 0 or ceiling_cost > 0:
            ...
            time_pct = (
                (time_savings / ceiling_timing * 100) if ceiling_timing > 0 else 0
            )
            lines.append(
                f"  Time:  {ceiling_timing:>8.1f}s       {actual_timing:>8.1f}s          {time_savings:>6.1f}s ({time_pct:.0f}%)"
            )

            cost_savings = ceiling_cost - actual_cost
            cost_pct = (cost_savings / ceiling_cost * 100) if ceiling_cost > 0 else 0
            lines.append(
                f"  Cost:  ${ceiling_cost:>7.3f}        ${actual_cost:>7.3f}          ${cost_savings:>5.3f} ({cost_pct:.0f}%)"
            )
        else:
        ...
        if opt_savings:
            ...
            lines.append("-" * 50)
            lines.append(
                f"  TOTAL: ${total_opt_cost:.3f} saved, {total_opt_time:.1f}s saved"
            )
        ...
        if batch_efficiency.get("total_batches", 0) > 0:
            ...
            lines.append(
                f"  Cold starts avoided: {batch_efficiency['cold_starts_avoided']}"
            )
            lines.append(
                f"  Amortization: {batch_efficiency['cold_start_amortization_pct']:.0f}%"
            )
