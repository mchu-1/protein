...
def _calculate_step_cost(
    step: str, duration_sec: float, gpu_override: Optional[str] = None
) -> float:
...
def _calculate_downstream_savings(
    ...
    if skipped_at_stage == "backbone":
        ...
        proteinmpnn_cost = _calculate_step_cost(
            "proteinmpnn", proteinmpnn_timeout, config.hardware.proteinmpnn_gpu
        )
        ...
        if config.esmfold.enabled:
            esmfold_cost = (
                _calculate_step_cost(
                    "esmfold", esmfold_timeout, config.hardware.esmfold_gpu
                )
                * num_seqs
            )
        ...
        boltz2_cost = (
            _calculate_step_cost("boltz2", boltz2_timeout, config.hardware.boltz2_gpu)
            * num_seqs
        )

        # Chai-1 cost for all sequence × decoy pairs
        chai1_cost = (
            _calculate_step_cost("chai1", chai1_timeout, config.hardware.chai1_gpu)
            * num_seqs
            * num_decoys
        )
        ...
        saved_time = (
            proteinmpnn_timeout
            + (esmfold_timeout * num_seqs)
            + (boltz2_timeout * num_seqs)
            + (chai1_timeout * num_seqs * num_decoys)
        )
        ...
        esmfold_cost = 0.0
        if config.esmfold.enabled:
            esmfold_cost = _calculate_step_cost(
                "esmfold", esmfold_timeout, config.hardware.esmfold_gpu
            )

        boltz2_cost = _calculate_step_cost(
            "boltz2", boltz2_timeout, config.hardware.boltz2_gpu
        )
        chai1_cost = (
            _calculate_step_cost("chai1", chai1_timeout, config.hardware.chai1_gpu)
            * num_decoys
...
def estimate_cost(config: PipelineConfig) -> dict:
    ...
    if config.adaptive.enabled:
        ...
        num_batches = math.ceil(
            config.rfdiffusion.num_designs / config.adaptive.batch_size
        )
    ...
    costs["rfdiffusion"] = _calculate_step_cost(
        "rfdiffusion", rfdiffusion_sec, config.hardware.rfdiffusion_gpu
    )

    # ProteinMPNN (L4) - timeout per backbone
    proteinmpnn_sec = proteinmpnn_timeout * effective_backbones
    costs["proteinmpnn"] = _calculate_step_cost(
        "proteinmpnn", proteinmpnn_sec, config.hardware.proteinmpnn_gpu
    )

    # ESMFold (T4) - timeout per sequence (if enabled)
    esmfold_sec = esmfold_timeout * effective_esmfold if config.esmfold.enabled else 0
    costs["esmfold"] = (
        _calculate_step_cost("esmfold", esmfold_sec, config.hardware.esmfold_gpu)
        if config.esmfold.enabled
        else 0.0
    )
    ...
    costs["boltz2"] = _calculate_step_cost(
        "boltz2", boltz2_sec, config.hardware.boltz2_gpu
    )
...
def _log_stage_metrics(
    ...
    """
    actual_cost = _calculate_step_cost(
        stage_name.lower().replace("-", ""), duration_sec
    )
    utilization = (duration_sec / ceiling_sec * 100) if ceiling_sec > 0 else 0

    print(
        f"  ├─ Duration: {duration_sec:.1f}s / {ceiling_sec:.0f}s ({utilization:.0f}% of ceiling)"
    )
...
def run_pipeline(config: PipelineConfig, use_mocks: bool = False) -> PipelineResult:
    ...
    print(
        f"│  └─ Sequences: {config.proteinmpnn.num_sequences}/backbone → {total_sequences} total"
    )
    ...
    print(
        f"  ├─ RFDiffusion ({config.hardware.rfdiffusion_gpu}):  ${cost_estimate['rfdiffusion']:.3f}"
    )
    print(
        f"  ├─ ProteinMPNN ({config.hardware.proteinmpnn_gpu}):    ${cost_estimate['proteinmpnn']:.3f}"
    )
    if config.esmfold.enabled:
        print(
            f"  ├─ ESMFold ({config.hardware.esmfold_gpu}):       ${cost_estimate['esmfold']:.3f}"
        )
    print(
        f"  ├─ Boltz-2 ({config.hardware.boltz2_gpu}):      ${cost_estimate['boltz2']:.3f}"
    )
    print(f"  ├─ FoldSeek (CPU):      ${cost_estimate['foldseek']:.3f}")
    print(
        f"  └─ Chai-1 ({config.hardware.chai1_gpu}):       ${cost_estimate['chai1']:.3f}"
    )
    ...
    beam_pruner = (
        BeamPruner(config.beam_pruning) if config.beam_pruning.enabled else None
    )
    ...
    if config.adaptive.enabled and not use_mocks:
        ...
        print(
            f"Adaptive generation complete: {len(backbones)} backbones, {len(sequences)} sequences, {len(predictions)} validated"
        )
        ...
        if not predictions:
            return _create_empty_result(
                run_id, config, validation_results, start_time, state_tree
            )
    else:
        # Standard batch generation
        print("\n=== Phase 1: Backbone Generation (RFDiffusion) ===")
        print(
            f"[START] {time.strftime('%H:%M:%S')} | Generating {config.rfdiffusion.num_designs} backbones"
        )
        stage_start = time.time()
        ...
        stage_duration = time.time() - stage_start
        print(
            f"[DONE]  {time.strftime('%H:%M:%S')} | Output: {len(backbones)} backbones | Duration: {stage_duration:.1f}s"
        )
        stage_metrics.append(
            _log_stage_metrics(
                ...
                stage_duration,
                len(backbones),
                cost_estimate["_timeouts"]["rfdiffusion"],
                cost_estimate["rfdiffusion"],
            )
        )
        ...
        for backbone in backbones:
            ...
            state_tree.set_ceiling_from_stage(
                backbone_node_id, cost_estimate["_timeouts"]["rfdiffusion"]
            )
        ...
        if config.structural_memoization.enabled and backbones:
            ...
            if twins_skipped > 0:
                print(
                    f"[OPT] Structural memoization: skipped {twins_skipped} structural twins"
                )

                # Calculate downstream savings using configured timeouts (consistent with ceiling)
                saved_cost_per_bb, saved_time_per_bb = _calculate_downstream_savings(
                    config, "backbone"
                )
        ...
        print(
            f"[START] {time.strftime('%H:%M:%S')} | Input: {len(backbones)} backbones × {config.proteinmpnn.num_sequences} seqs each"
        )
        stage_start = time.time()
        ...
        stage_duration = time.time() - stage_start
        print(
            f"[DONE]  {time.strftime('%H:%M:%S')} | Output: {len(sequences)} sequences | Duration: {stage_duration:.1f}s"
        )
        stage_metrics.append(
            _log_stage_metrics(
                "proteinmpnn",
                stage_duration,
                len(sequences),
                cost_estimate["_timeouts"]["proteinmpnn"] * len(backbones),
                cost_estimate["proteinmpnn"],
            )
        )
        ...
        if state_tree and sequences:
            ...
            for seq in sequences:
                ...
                state_tree.set_ceiling_from_stage(
                    seq_node_id, cost_estimate["_timeouts"]["proteinmpnn"]
                )
        ...
        optimization_stats["solubility_filtered"] = filter_stats.get(
            "solubility_filtered", 0
        )
        optimization_stats["stickiness_filtered"] = filter_stats.get(
            "stickiness_filtered", 0
        )
        ...
        if total_filtered > 0:
            print(
                f"[OPT] Sequence filters: {pre_filter_count} → {len(sequences)} sequences"
            )
            print(
                f"  ├─ Solubility: {filter_stats.get('solubility_filtered', 0)} filtered ({filter_stats.get('solubility_timing_sec', 0):.2f}s)"
            )
            print(
                f"  ├─ Stickiness: {filter_stats.get('stickiness_filtered', 0)} filtered ({filter_stats.get('stickiness_timing_sec', 0):.2f}s)"
            )
            print(
                f"  └─ Beam pruning: {filter_stats.get('beam_pruned', 0)} pruned ({filter_stats.get('beam_pruning_timing_sec', 0):.2f}s)"
            )
            print(
                f"  Total filter time: {filter_stats.get('total_timing_sec', 0):.2f}s"
            )

            # Calculate downstream savings using configured timeouts (consistent with ceiling)
            saved_cost_per_seq, saved_time_per_seq = _calculate_downstream_savings(
                config, "sequence"
            )
            ...
            state_tree.set_metrics(
                run_id,
                {
                    "sequence_filter_timing": {
                        ...
                        "beam_pruning_sec": filter_stats.get(
                            "beam_pruning_timing_sec", 0
                        ),
        ...
        if config.esmfold.enabled and not use_mocks:
            print("\n=== Phase 1.5: ESMFold Gatekeeper (Orthogonal Validation) ===")
            print(
                f"[START] {time.strftime('%H:%M:%S')} | Input: {len(sequences)} sequences"
            )
            print(
                f"  Thresholds: pLDDT ≥ {config.esmfold.min_plddt}, RMSD ≤ {config.esmfold.max_rmsd} Å"
            )
            stage_start = time.time()
            ...
            sequences = run_esmfold_validation_batch.remote(
                ...
                output_dir=f"{dirs['sequences']}/esmfold"
                if config.esmfold.save_predictions
                else None,
            )
            ...
            prune_rate = (
                100 * pruned_count / pre_esmfold_count if pre_esmfold_count > 0 else 0
            )

            print(
                f"[DONE]  {time.strftime('%H:%M:%S')} | Output: {post_esmfold_count}/{pre_esmfold_count} passed ({pruned_count} pruned, {prune_rate:.1f}%)"
            )
            ...
            if pruned_count > 0:
                ...
                print(
                    f"  Boltz-2 + Chai-1 avoided: ${saved_cost_per_seq * pruned_count:.2f}"
                )
        ...
        print(
            f"[START] {time.strftime('%H:%M:%S')} | Input: {len(sequences_to_validate)} sequences"
        )
        stage_start = time.time()
        ...
        stage_duration = time.time() - stage_start
        print(
            f"[DONE]  {time.strftime('%H:%M:%S')} | Output: {len(predictions)} passed | Duration: {stage_duration:.1f}s"
        )
        stage_metrics.append(
            _log_stage_metrics(
                "boltz2",
                stage_duration,
                len(predictions),
                cost_estimate["_timeouts"]["boltz2"] * len(sequences_to_validate),
                cost_estimate["boltz2"],
            )
        )
        ...
        for i, pred in enumerate(predictions):
            ...
            # Set ceiling cost based on stage timeout (passive observability)
            state_tree.set_ceiling_from_stage(
                pred_node_id, cost_estimate["_timeouts"]["boltz2"]
            )
            state_tree.end_timing(
                pred_node_id,
                status=NodeStatus.COMPLETED,
    ...
    print(
        f"[START] {time.strftime('%H:%M:%S')} | Searching for up to {config.foldseek.max_hits} decoys"
    )
    stage_start = time.time()
    ...
    stage_duration = time.time() - stage_start
    print(
        f"[DONE]  {time.strftime('%H:%M:%S')} | Output: {len(decoys)} decoys | Duration: {stage_duration:.1f}s"
    )
    stage_metrics.append(
        _log_stage_metrics(
            "foldseek",
            stage_duration,
            len(decoys),
            cost_estimate["_timeouts"]["foldseek"],
            cost_estimate["foldseek"],
        )
    )
    ...
    for decoy in decoys:
        ...
        # Set ceiling cost based on stage timeout (passive observability)
        state_tree.set_ceiling_from_stage(
            decoy_node_id, cost_estimate["_timeouts"]["foldseek"]
        )
        state_tree.end_timing(
            decoy_node_id,
            status=NodeStatus.COMPLETED,
    ...
    if validated_sequences:
        ...
        print(
            f"[START] {time.strftime('%H:%M:%S')} | Input: {len(validated_sequences)} sequences × {num_decoys} decoys"
        )
        stage_start = time.time()

        # Run positive control + decoy check
        positive_control_results, cross_reactivity_results = (
            _run_cross_reactivity_check(
                validated_sequences,
        ...
        num_binding = sum(
            1 for r in positive_control_results.values() if r.plddt_interface > 50
        )
        print(
            f"[DONE]  {time.strftime('%H:%M:%S')} | Positive control: {num_binding}/{len(validated_sequences)} bind target | Duration: {stage_duration:.1f}s"
        )

        if decoys:
            print(
                f"        Decoy check: {len(cross_reactivity_results)} sequences checked against {num_decoys} decoys"
            )
        ...
        stage_metrics.append(
            _log_stage_metrics(
                ...
                stage_duration,
                num_pairs,
                cost_estimate["_timeouts"]["chai1"]
                * cost_estimate["_effective"]["chai1_pairs"],
                cost_estimate["chai1"],
            )
        )
        ...
        for seq_id, pc_result in positive_control_results.items():
            ...
            # Set ceiling cost based on stage timeout (passive observability)
            state_tree.set_ceiling_from_stage(
                pc_node_id, cost_estimate["_timeouts"]["chai1"]
            )
        ...
        for seq_id, cr_results in cross_reactivity_results.items():
            for cr in cr_results:
                ...
                # Set ceiling cost based on stage timeout (passive observability)
                state_tree.set_ceiling_from_stage(
                    cr_node_id, cost_estimate["_timeouts"]["chai1"]
                )
    ...
    for prediction in predictions:
        ...
        if pc_result:
            ...
            chai1_specificity_score = (
                chai1_ppi * 100 if chai1_ppi > 0 else pc_result.plddt_interface
            )
        ...
        boltz_specificity = (
            ppi_score * 100 if ppi_score > 0 else prediction.plddt_interface
        )
        ...
        if chai1_specificity_score is not None and chai1_specificity_score > 0:
            ...
            print(
                f"  Consensus scoring for {sequence.sequence_id}: Boltz={specificity_score:.1f}, Chai={chai1_specificity_score:.1f} -> {consensus_specificity:.1f}"
            )
    ...
    for stage in ["rfdiffusion", "proteinmpnn", "boltz2", "foldseek", "chai1"]:
        ...
        if stage == "rfdiffusion" and config.adaptive.enabled:
            ...
            ceiling_time = (
                cost_estimate["_timeouts"].get(stage, 0)
                * cost_estimate["_effective"]["backbones"]
            )
        elif stage == "boltz2":
            ceiling_time = (
                cost_estimate["_timeouts"].get(stage, 0)
                * cost_estimate["_effective"]["sequences"]
            )
        elif stage == "chai1":
            ceiling_time = (
                cost_estimate["_timeouts"].get(stage, 0)
    ...
    print("-" * 60)
    print(
        f"{'TOTAL':<15} {actual_timing:.1f}s / {cost_estimate['_runtime_sec']:.0f}s      ${actual_cost:.3f} / ${ceiling_cost:.3f}"
    )

    savings = ceiling_cost - actual_cost
    if savings > 0:
        print(
            f"  Savings: ${savings:.3f} ({savings / ceiling_cost * 100:.0f}% under ceiling)"
        )
    ...
    if opt_savings or batch_efficiency.get("total_batches", 0) > 0:
        ...
        if opt_savings:
            print(
                f"{'Filter/Optimization':<25} {'Items':<10} {'Cost Saved':<15} {'Time Saved':<15}"
            )
            print("-" * 65)
            ...
            print("-" * 65)
            print(
                f"{'TOTAL':<25} {'':<10} ${total_opt_cost:<14.3f} {total_opt_time:<14.1f}s"
            )

        if batch_efficiency.get("total_batches", 0) > 0:
            print("\nBatch Consolidation:")
            print(
                f"  Batches: {batch_efficiency['total_batches']} (avg size: {batch_efficiency['avg_batch_size']:.1f})"
            )
            print(f"  Cold starts avoided: {batch_efficiency['cold_starts_avoided']}")
            print(
                f"  Amortization efficiency: {batch_efficiency['cold_start_amortization_pct']:.0f}%"
            )
    ...
    state_tree.set_metrics(
        run_id,
        {
            ...
            "best_candidate_id": top_candidates[0].candidate_id
            if top_candidates
            else None,
            "ceiling_cost_usd": ceiling_cost,
            "savings_usd": savings if savings > 0 else 0,
            "savings_pct": (savings / ceiling_cost * 100)
            if ceiling_cost > 0 and savings > 0
            else 0,
            # Optimization metrics
            "optimizations": {
                "structural_twins_skipped": optimization_stats.get(
                    "structural_twins_skipped", 0
                ),
    ...
    _write_stage_metrics_from_tree(
        campaign_dir, state_tree, cost_estimate, runtime_seconds
    )
...
def _run_adaptive_generation(
    ...
    print(
        f"Adaptive mode: target {adaptive_config.min_validated_candidates} validated candidates"
    )
    print(
        f"  Micro-batch size: {adaptive_config.batch_size} backbones, max batches: {max_batches} (from {rfdiffusion_config.num_designs} total designs)"
    )
    print("  (GPU-efficient: full batch processing, early termination between batches)")

    while (
        len(validated_predictions) < adaptive_config.min_validated_candidates
        ...
        try:
            batch_backbones = run_rfdiffusion.remote(
                target, batch_rfdiffusion_config, batch_output_dir
            )
        except Exception as e:
        ...
        if state_tree and target_node_id:
            ...
            avg_duration = (
                batch_duration / len(batch_backbones) if batch_backbones else 0.0
            )
            ...
            for i, backbone in enumerate(batch_backbones):
                ...
                if backbone_node_id in state_tree.graph:
                    ...
                    node_data.timing.end_time = (
                        node_data.timing.start_time + avg_duration
                    )
                    node_data.timing.duration_sec = avg_duration

                state_tree.end_timing(backbone_node_id, NodeStatus.COMPLETED)
                state_tree.set_batch_info(
                    backbone_node_id, batch_id, i, len(batch_backbones)
                )

            # Mark filtered backbones
            for bb_id in pre_filter_bb_ids - kept_bb_ids:
                if config:
                    saved_cost, saved_time = _calculate_downstream_savings(
                        config, "backbone"
                    )
                    state_tree.mark_skipped(
                        node_id=bb_id,
        ...
        print(
            f"  Step 2: Designed {len(batch_sequences)} sequences ({len(batch_sequences) // len(batch_backbones)} per backbone)"
        )
        ...
        if solubility_config or stickiness_config or beam_pruner:
            ...
            if pre_filter > len(batch_sequences):
                print(
                    f"  [OPT] Sequence filters: {pre_filter} → {len(batch_sequences)}"
                )
        ...
        if state_tree:
            ...
            avg_duration = (
                batch_duration / len(batch_sequences) if batch_sequences else 0.0
            )
            ...
            for i, seq in enumerate(batch_sequences):
                ...
                state_tree.set_batch_info(
                    seq_node_id, seq_batch_id, i, len(batch_sequences)
                )
        ...
        if config and config.esmfold.enabled:
            ...
            try:
                batch_sequences = run_esmfold_validation_batch.remote(
                    ...
                    min_plddt=config.esmfold.min_plddt,
                    max_rmsd=config.esmfold.max_rmsd,
                    output_dir=f"{dirs['sequences']}/esmfold/batch_{batch_num}"
                    if config.esmfold.save_predictions
                    else None,
                )
            except Exception as e:
            ...
            if pruned_count > 0:
                print(
                    f"  [ESMFold] Gatekeeper: {pre_esmfold_count} → {post_esmfold_count} sequences ({pruned_count} pruned)"
                )
        ...
        if max_boltz2_per_batch and len(batch_sequences) > max_boltz2_per_batch:
            ...
            batch_sequences_ranked = sorted(
                batch_sequences, key=lambda s: s.score, reverse=True
            )
            ...
            print(
                f"  [LIMIT] Boltz-2: {len(batch_sequences)} → {max_boltz2_per_batch} seqs (top by score: {', '.join(top_scores)})"
            )
        else:
        ...
        if batch_results:
            ...
            if state_tree:
                ...
                for i, (seq, pred, error) in enumerate(batch_results):
                    ...
                    if error and (
                        "failed" in error.lower() or "timeout" in error.lower()
                    ):
                    ...
                    state_tree.set_batch_info(
                        pred_node_id, val_batch_id, i, len(batch_results)
                    )
            ...
            print(
                f"  Step 3: Validated {len(batch_predictions)}/{len(sequences_for_validation)} sequences"
            )
            print(
                f"  Running total: {len(validated_predictions)}/{adaptive_config.min_validated_candidates} validated candidates"
            )
        else:
    ...
    if batch_num < max_batches:
        ...
        if saved_backbones > 0 or saved_sequences > 0:
            ...
            print(
                f"    Skipped: {saved_backbones} backbones, {saved_sequences} sequences"
            )
            # Estimate cost savings (rough)
            # Use configured hardware prices
            import pipeline

            rfd_cost = pipeline.MODAL_GPU_COST_PER_SEC.get(
                config.hardware.rfdiffusion_gpu, 0.000306
            )
            mpnn_cost = pipeline.MODAL_GPU_COST_PER_SEC.get(
                config.hardware.proteinmpnn_gpu, 0.000222
            )
            boltz_cost = pipeline.MODAL_GPU_COST_PER_SEC.get(
                config.hardware.boltz2_gpu, 0.000583
            )
            ...
            saved_cost = saved_backbones * backbone_cost_per + saved_sequences * (
                sequence_cost_per + validation_cost_per
            )
...
def _run_sequence_design(
    ...
    if use_mocks:
        all_sequences = []
        for backbone in backbones:
            seqs = mock_proteinmpnn(
                backbone, config, f"{output_dir}/{backbone.design_id}"
            )
...
def _run_structure_validation(
    ...
    try:
        results = validate_sequences_parallel.remote(
            sequences, target, config, output_dir
        )
        ...
        if config.cache_results:
            ...
            if cached_decoys is not None:
                ...
                if len(valid_decoys) == len(cached_decoys):
                    print(
                        f"[OPT] FoldSeek cache HIT: saved {foldseek_time}s, ${foldseek_cost:.4f}"
                    )
        ...
        for seq in sequences:
            ...
            for decoy in decoys:
                result = mock_chai1(
                    ...
                    decoy,
                    config,
                    f"{output_dir}/{seq.sequence_id}/{decoy.decoy_id}",
                )
...
def _write_run_manifest(
    ...
    manifest = {
        ...
        "configuration": {
            ...
            "total_sequences": config.rfdiffusion.num_designs
            * config.proteinmpnn.num_sequences,
...
def _write_stage_metrics(
    campaign_dir: str, stage_metrics: list[dict], runtime_seconds: float
) -> None:
    ...
    metrics = {
        ...
        "savings_pct": (total_ceiling - total_actual) / total_ceiling * 100
        if total_ceiling > 0
        else 0,
...
def _write_stage_metrics_from_tree(
    ...
    metrics = {
        ...
        "savings_pct": (
            (cost_estimate["total"] - state_tree.get_total_cost())
            / cost_estimate["total"]
            * 100
        ...
        for c in candidates:
            ...
            writer.writerow(
                [
                    ...
                    f"{c.chai1_specificity_score / 100:.3f}"
                    if c.chai1_specificity_score
                    else "",
...
def print_dry_run_summary(config: PipelineConfig) -> None:
    ...
    esmfold_sec = (
        timeouts.get("esmfold", 0) * eff_esmfold if config.esmfold.enabled else 0
    )
    ...
    print(
        f"│        └─ Chai-1: {eff_chai1_seqs} seqs × {eff_decoys} decoys = {eff_chai1} pairs"
    )
    ...
    print(
        f"{'RFDiffusion':<20} {config.hardware.rfdiffusion_gpu:<12} {eff_backbones:<10} {rfdiffusion_sec:>6}s {'$' + f'{cost_rfd:.3f}':>12}"
    )
    print(
        f"{'ProteinMPNN':<20} {config.hardware.proteinmpnn_gpu:<12} {eff_backbones:<10} {proteinmpnn_sec:>6}s {'$' + f'{cost_mpnn:.3f}':>12}"
    )
    if config.esmfold.enabled:
        print(
            f"{'ESMFold':<20} {config.hardware.esmfold_gpu:<12} {eff_esmfold:<10} {esmfold_sec:>6}s {'$' + f'{cost_esmfold:.3f}':>12}"
        )
    print(
        f"{'Boltz-2':<20} {config.hardware.boltz2_gpu:<12} {eff_boltz2:<10} {boltz2_sec:>6}s {'$' + f'{cost_boltz:.3f}':>12}"
    )
    print(
        f"{'FoldSeek':<20} {'CPU':<12} {1:<10} {foldseek_sec:>6}s {'$' + f'{cost_fseek:.3f}':>12}"
    )
    print(
        f"{'Chai-1':<20} {config.hardware.chai1_gpu:<12} {eff_chai1:<10} {chai1_sec:>6}s {'$' + f'{cost_chai:.3f}':>12}"
    )
    print("-" * 70)
    print(
        f"{'CEILING TOTAL':<20} {'':<12} {'':<10} {int(est_runtime_sec):>6}s {'$' + f'{cost_total:.2f}':>12}"
    )
    print()
    ...
    opt_settings = [
        (
            "ESMFold Gatekeeper",
            config.esmfold.enabled,
            f"pLDDT≥{config.esmfold.min_plddt}, RMSD≤{config.esmfold.max_rmsd}Å",
        ),
        (
            "Structural Memoization",
            config.structural_memoization.enabled,
            f"threshold={config.structural_memoization.similarity_threshold}",
        ),
        (
            "Solubility Filter",
            config.solubility_filter.enabled,
            f"forbidden_pI={config.solubility_filter.forbidden_pi_min}-{config.solubility_filter.forbidden_pi_max}",
        ),
        (
            "Stickiness Filter (SAP)",
            config.stickiness_filter.enabled,
            f"max_sap={config.stickiness_filter.max_sap_score}",
        ),
        (
            "Beam Pruning",
            config.beam_pruning.enabled,
            f"max_nodes={config.beam_pruning.max_tree_nodes}, seqs/bb={config.beam_pruning.sequences_per_backbone}",
        ),
    ]
...
def main(
    ...
    if config is not None:
        ...
        if dry_run:
            pipeline_config.target.pdb_path = (
                f"/data/targets/{pdb_id.lower()}.pdb"  # Placeholder
            )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            remote_pdb_path = upload_target_pdb.remote(
                pdb_content, f"{pdb_id.lower()}.pdb"
            )
    ...
    if result.top_candidates:
        ...
        for i, candidate in enumerate(result.top_candidates[:5]):
            ...
            if candidate.decoy_results:
                binding_decoys = sum(
                    1 for r in candidate.decoy_results if r.binds_decoy
                )
                print(
                    f"   Binds {binding_decoys}/{len(candidate.decoy_results)} decoys"
                )
...
Examples:
  ...
  modal run pipeline.py --config config.yaml
    ...
    parser.add_argument(
        "--mode", default="bind", help="Generation mode (default: bind)"
    )
    parser.add_argument(
        "--num-designs", type=int, default=5, help="Number of backbones (default: 5)"
    )
    parser.add_argument(
        "--num-sequences",
        type=int,
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Preview costs without running"
    )
    ...
    if args.config is None and (
        args.pdb_id is None or args.entity_id is None or args.hotspot_residues is None
    ):
        print(
            "Error: Either --config must be provided, or all of: --pdb-id, --entity-id, --hotspot-residues"
        )
        sys.exit(1)
    ...
    if args.dry_run:
        ...
        if args.config:
            ...
            if not chains:
                print(
                    f"Error: No chains found for entity {args.entity_id} in PDB {pdb_id}"
                )
                sys.exit(1)

            chain_id = chains[0]
            entity_name = entity_info.get(
                "description", f"{pdb_id}_entity{args.entity_id}"
            )
            uniprot_ids = entity_info.get("uniprot_ids", [])
        ...
        if args.config:
            ...
            print("Error: For actual pipeline runs, use 'modal run':")
            print(
                f"  modal run pipeline.py --pdb-id {args.pdb_id} --entity-id {args.entity_id} \\"
            )
            print(f'    --hotspot-residues "{args.hotspot_residues}"')
            print("\nFor dry-run (cost estimation only):")
            print(
                f"  python pipeline.py --pdb-id {args.pdb_id} --entity-id {args.entity_id} \\"
            )
